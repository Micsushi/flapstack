...
try {
  ...
  const metadata = runState.assistant?.metadata ?? null
  const usage =
    metadata?.usage ??
    (typeof metadata?.totalTokens === "number"
      ...
      : null)
  assert(
    assistantText?.endsWith("ORCHESTRATION_FORK_OK"),
    "Provider response did not contain the terminal proof token",
  )
  assert(
    ...
    JSON.stringify(
      {
        ...
        proof: {
          ...
          lineage: {
            rendererRendered: Boolean(childRenderer?.card && parentRenderer?.card),
