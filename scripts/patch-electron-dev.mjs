// Patches the Electron.app bundle in node_modules to show "Flapstack" name and icon in macOS dock during dev mode.
...
if (existsSync(plistPath)) {
  try {
    execSync(`/usr/libexec/PlistBuddy -c "Set :CFBundleName Flapstack" "${plistPath}"`)
    execSync(`/usr/libexec/PlistBuddy -c "Set :CFBundleDisplayName Flapstack" "${plistPath}"`)
    console.log("[patch-electron-dev] Updated Info.plist: name -> Flapstack")
  } catch (e) {
