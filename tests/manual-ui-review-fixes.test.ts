...
describe("manual UI review fixes", () => {
  ...
  it("offers durable group organization and shared workbench history", () => {
    ...
    const appCommandBridge = read("src/renderer/components/app-command-bridge.tsx")
    ...
    expect(appCommandBridge).toContain("event.stopImmediatePropagation()")
    expect(appCommandBridge).toContain('platform === "darwin"')
    expect(appCommandBridge).toContain("isEditableTarget(event.target)")
