...
import { resolveChatStatus, summarizeChatStatuses } from "../src/renderer/features/agents/lib/chat-status"
...
describe("Independent read, runtime and work status", () => {
  it.each(["success", "completed", "ready", null, "unexpected"])("does not treat %s as verified work", (runStatus) => {
    expect(resolveChatStatus({ runStatuses: [runStatus] }).outcome).toBe("unknown")
  })

  it.each([
    ["running", "unknown"], ["needs-input", "unknown"], ["cancelled", "stopped-incomplete"],
    ["blocked", "blocked"], ["failure", "failed"], [null, "unknown"],
    ...
    const after = resolveChatStatus({ unread: markChatSeen(unread, "chat").has("chat"), runStatuses: [runStatus] })
  ...
  it("keeps simultaneous running, needs-input, and failed group members visible", () => {
    ...
    expect(summary).toEqual({ unread: 1, running: 1, needsHelp: 1, outcomes: ["unknown", "failed", "stopped-incomplete"] })
  })
  ...
  it("restores saved terminal truth without inventing verification", () => {
    ...
    expect(resolveChatStatus({ running: true, needsHelp: true, runStatuses: ["failure"] })).toMatchObject({ running: true, needsHelp: true, outcome: "failed" })
  })
})
