...
describe("saved workspace schema migration", () => {
  ...
  it("upgrades the supported 0033 schema additively and idempotently", () => {
    const { sqlite, directory } = database("prior-schema")
    try {
      migrate(drizzle(sqlite, { schema }), { migrationsFolder: migrationSubset(directory, 32) })
      seedExistingWork(sqlite)
      const chatBefore = existingChat(sqlite)
  ...
  it("keeps saved workspaces in the authoritative migration slot", () => {
    ...
    expect(journal.entries.find((entry) => entry.tag === "0033_saved_workspaces")).toEqual(
      expect.objectContaining({ idx: 33, tag: "0033_saved_workspaces" }),
    )
    expect(journal.entries.find((entry) => entry.idx === 34)?.tag).toBe("0034_agent_runtime")
  })
})
