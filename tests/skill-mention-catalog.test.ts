...
import { extensionPolicyTargetFromManifest, setExtensionEnablementPolicy } from "../src/main/lib/extension-management/enablement-policy"
...
describe("native composer skill discovery", () => {
  ...
  beforeEach(() => {
    ...
    database.insert(schema.projects).values({ id: "other", name: "Other", path: join(root, "other") }).run()
    database.insert(schema.tasks).values({ id: "t", name: "Task", projectId: "p" }).run()
    database.insert(schema.chats).values({ id: "c", projectId: "p", taskId: "t", scope: "task", harness: "claude-code" }).run()
    database.insert(schema.subChats).values({ id: "s", chatId: "c", harness: "codex", worktreePath: projectRoot }).run()
  })
  ...
  it("reads native user/project skills for the exact pane runtime and scope", async () => {
    ...
    const result = await listNativeSkillMentions(database, { subChatId: "s", projectId: "other", harness: "claude-code" }, { homeDir })
    expect(result.harness).toBe("codex")
    expect(result.skills.map((skill) => skill.name)).toEqual(["project", "legacy", "user"])
    expect(result.skills[0]).toMatchObject({ source: "project", provider: "codex", compatibility: false })
    expect(result.skills[1]).toMatchObject({ source: "project", provider: "codex", compatibility: true })
  })
  ...
  it("honors task policy and never revives a disabled native skill through a compatibility alias", async () => {
    ...
    const native = (await discoverProviderExtensions({ homeDir, cwd: projectRoot })).find((entry) => entry.kind === "skill" && entry.name === "review" && entry.capabilities.discovery === "supported")!
    setExtensionEnablementPolicy(database, {
      target: extensionPolicyTargetFromManifest(native), location: { type: "task", projectId: "p", taskId: "t" }, enabled: false,
    })
    expect((await listNativeSkillMentions(database, { subChatId: "s" }, { homeDir })).skills).toEqual([])
    const draft = await listNativeSkillMentions(database, { projectId: "p", harness: "codex" }, { homeDir })
  ...
  it("validates draft task/project scope and returns no guessed inventory without a runtime", async () => {
    await expect(listNativeSkillMentions(database, { projectId: "other", taskId: "t", harness: "codex" }, { homeDir })).rejects.toThrow("Task does not belong")
    expect((await listNativeSkillMentions(database, {}, { homeDir })).skills).toEqual([])
    await expect(listNativeSkillMentions(database, { subChatId: "missing" }, { homeDir })).rejects.toThrow("Chat not found")
  })
})
