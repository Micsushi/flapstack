...
import { ChatStatusIndicators, ChatStatusProvider, ChatStatusSummary } from "../src/renderer/features/agents/ui/chat-status-indicators"
import { outcomeLabels, type WorkOutcome } from "../src/renderer/features/agents/lib/chat-status"

vi.mock("../src/renderer/lib/trpc", () => ({ trpc: { chats: { listAgentMetadata: { useQuery: () => ({ data: { runStatuses: [
...
describe("Chat status accessibility", () => {
  it("combines persisted run truth with independent live input, runtime, and unread", () => {
    const html = renderToStaticMarkup(<ChatStatusProvider><ChatStatusIndicators chatIds={["a", "b", "a"]} /></ChatStatusProvider>)
    expect(html).toContain("1 unread; 1 running; 1 needs help or input; Run failed; work not verified; Work outcome unknown")
    expect(html).not.toContain("Verified work complete")
  })
  it.each(Object.keys(outcomeLabels) as WorkOutcome[])("labels %s independently of unread and running", (outcome) => {
    for (const unread of [false, true]) {
      const html = renderToStaticMarkup(<ChatStatusSummary statuses={[{ unread, running: true, needsHelp: true, outcome }]} />)
  ...
  it("preserves incomplete and unknown members beside verified members", () => {
    const html = renderToStaticMarkup(<ChatStatusSummary statuses={[
