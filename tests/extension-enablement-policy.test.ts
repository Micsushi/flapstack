...
describe("extension enablement policy", () => {
  ...
  it("upgrades an existing 0031 profile additively and idempotently", () => {
    const legacyMigrations = join(directory, "migrations-through-0031")
    ...
    writeFileSync(
      journalPath,
      `${JSON.stringify({ ...journal, entries: journal.entries.filter((entry) => entry.idx <= 31) }, null, 2)}\n`,
    )
  ...
  it("keeps the migration serialized as 0032 immediately after 0031", () => {
    const journal = JSON.parse(readFileSync(join(migrations, "meta", "_journal.json"), "utf8")) as {
      entries: Array<{ idx: number; tag: string }>
    }
    const advancedUsageIndex = journal.entries.findIndex((entry) => entry.idx === 31)
    ...
    expect(journal.entries.slice(advancedUsageIndex, advancedUsageIndex + 2)).toEqual([
      expect.objectContaining({ idx: 31, tag: "0031_advanced_usage_contracts" }),
      expect.objectContaining({ idx: 32, tag: "0032_extension_enablement_policy" }),
    ])
  })
