...
describe("0036 Agent Profile migration", () => {
  it("is canonical, preserves legacy rows, and installs immutable history", () => {
    ...
    expect(journal.entries.at(-1)).toEqual(
      expect.objectContaining({ idx: 36, tag: "0036_agent_profiles" }),
    )
    expect(readFileSync(resolve(migrations, "0036_agent_profiles.sql"), "utf8")).toContain(
    ...
    rmSync(resolve(legacyDirectory, "0036_agent_profiles.sql"))
    rmSync(resolve(legacyDirectory, "meta/0036_snapshot.json"))
    writeFileSync(
      resolve(legacyDirectory, "meta/_journal.json"),
      JSON.stringify({ ...journal, entries: journal.entries.filter((entry) => entry.idx <= 35) }),
    )
