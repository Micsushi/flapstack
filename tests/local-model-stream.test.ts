...
describe("local model stream", () => {
  ...
  it("cancels a superseded sub-chat stream without overwriting the newer run", async () => {
    ...
    // Keep the replacement ID lexically lower while both runs share the same
    // second-level started_at value. Stream identity, not ID ordering, owns the
    // authoritative sub-chat status.
    const first = service.stream(input({ runId: "run-z-first", prompt: "first" }))
    await first.next()
    await first.next()
    await first.next()

    const replacement = await collect(
      service.stream(input({ runId: "run-a-replacement", prompt: "second" })),
    )
    ...
    expect(fixture.run("run-z-first")).toMatchObject({ status: "cancelled" })
    expect(fixture.run("run-a-replacement")).toMatchObject({ status: "success" })
