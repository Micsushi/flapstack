...
describe("mobile-pairing", () => {
  it("upgrades an existing 0030 profile with additive 0031 tables and metadata", () => {
    const legacy = new Database(":memory:")
    try {
      legacy.pragma("foreign_keys = ON")
      migrate(drizzle(legacy, { schema }), { migrationsFolder: migrationsThrough0030() })
  ...
  it("persists replay protection, handles concurrent nonce claims, and survives restart", async () => {
    const service = createService()
    const key = ed25519Key()
    ...
    expect(
      restarted.authenticateSession(sessionProof(credential, key.privateKey, "r".repeat(32))),
    ).toMatchObject({ sessionId: credential.session.sessionId })
  })
...
function migrationsThrough0030(): string {
  ...
  const subset = { ...journal, entries: journal.entries.filter((entry) => entry.idx <= 30) }
