# Stage 2 full-feature test matrix (historical)

Snapshot: 2026-07-11; superseded 2026-07-13. Stage 2 testing is closed as a
separate release gate. Every previously unchecked criterion is preserved below
as a migrated record and is now owned by
`docs/stage3-full-feature-test-matrix.md` plus the matching S3-F9/F14/F15/F16/
F17 OpenSpec task board. This file is not an active checklist.
...
- **MIGRATED** **P-01 READY** `npm ci --legacy-peer-deps` finishes with Kokoro and
      Transformers present.
      Verify with `npm ls kokoro-js @huggingface/transformers --depth=0`.
- **MIGRATED** **P-02 READY** `npm run check` passes lint, formatting, tests, and production
      build under Node 22.
- **MIGRATED** **P-03 READY** `npm run ts:check` passes and is part of `npm run check`
      and CI.
- **MIGRATED** **P-04 READY** strict OpenSpec validation passes.
- **MIGRATED** **P-05 READY (macOS)** the usage-daemon smoke writes a heartbeat/poll row,
      exits cleanly, and reports `usage daemon smoke passed`.
- **MIGRATED** **P-06 READY** the OpenCode lifecycle smoke launches the pinned sidecar,
      reaches health, creates a session, and tears down within five seconds.
- **MIGRATED** **P-07 READY** `git diff --check` reports no whitespace errors.
- **MIGRATED** **P-08 CONDITIONAL** packaged macOS app launches from Finder with the same
...
- **MIGRATED** **P-09 CONDITIONAL** packaged Windows app launches and finds every bundled
      or required voice/provider executable.
- **MIGRATED** **P-10 CONDITIONAL** dependency-audit findings are reviewed and either
...
- **MIGRATED** **F3-01 READY** Open Changes, open the branch menu, choose **Create new
...
- **MIGRATED** **F3-02 READY** Repeat with an invalid name such as `bad..name`. Expected:
      the dialog shows the backend validation error and does not create or switch.
- **MIGRATED** **F3-03 READY** Force a Git create/checkout failure. Expected: the dialog
      remains recoverable and the old branch stays selected.
- **MIGRATED** **F4-01 READY** Print a relative file link, an absolute file link, a
...
- **MIGRATED** **F4-02 READY** Run a command. Expected: its sanitized command becomes the
      terminal-tab title; secrets/control sequences do not become the title.
- **MIGRATED** **F4-03 READY** Focus terminals in sidebar, bottom panel, and details panel.
      Expected: the active pane/tab state follows focus in every surface.

### Remote changes

- **MIGRATED** **F5-01 READY** Open a remote sandbox chat in the desktop app. Expected:
...
- **MIGRATED** **F6-01 READY** For Claude and Codex, inspect every permission mode before
      launch. Expected: enforced controls and limitations are honest; no UI claims a
      stronger sandbox than the harness provides.
- **MIGRATED** **F6-02 READY** Launch a no-edit prompt in read-only mode and explicitly ask
...
- **MIGRATED** **F6-03 READY** Launch Codex with a plugin MCP server whose command path is
...
- **MIGRATED** **F7-01 READY** Select project default and task default worktrees. Expected:
      the selected cwd and worktree chip match the target.
- **MIGRATED** **F7-02 READY** Enter an absolute path to another valid Git checkout.
...
- **MIGRATED** **F7-03 READY** Try a relative path, missing path, regular file, and existing
...
- **MIGRATED** **F7-04 READY** Remove or invalidate a selected worktree after selection.
      Expected: the UI shows unknown/needs-refresh, not a false clean state.

### Search, attachments, runs, and scope movement

- **MIGRATED** **F8-01 READY** Create more than 20 scoped-search results. Expected: **Show
      more** reveals every result in pages of 20; include-archived and scope filters
      remain applied.
- **MIGRATED** **F8-02 READY** Search a deep user/assistant text match. Expected: clicking
...
- **MIGRATED** **F8-03 READY** Search standard `reasoning` output from Cursor/OpenCode.
...
- **MIGRATED** **F8-04 READY** Search matching project and task titles. Expected: rows
...
- **MIGRATED** **F9-01 READY** Add at least seven attachments. Expected: six-item preview,
      explicit **Show all**, and access to all attachments.
- **MIGRATED** **F9-02 READY** Promote file, image, and pasted-text attachments to a task.
      Expected: each remains visible under **Task artifacts** after switching chats.
- **MIGRATED** **F9-03 READY** Click **View artifact** for stored files/images and inline
...
- **MIGRATED** **F9-04 READY** Write an artifact to the selected worktree. Expected: path
      traversal and unknown roots are rejected; overwrite requires confirmation.
- **MIGRATED** **F10-01 READY** Produce more than five runs in one chat. Expected: preview
      shows five, **Show all** exposes every run, and **Show fewer** restores preview.
- **MIGRATED** **F10-02 READY** Inspect old runs. Expected: status, harness/model, prompt,
      checkpoint, and manifest links remain reachable where recorded.
- **MIGRATED** **F11-01 READY** From a local chat overflow menu and context menu, choose
...
- **MIGRATED** **F11-02 READY** Move global → project → task → global. Expected: history,
...
- **MIGRATED** **F11-03 READY** Move between tasks in different projects. Expected: target
...
- **MIGRATED** **V2-01 CONDITIONAL (packaged clean install)** With no speech cache and no
...
- **MIGRATED** **V2-02 READY (dev macOS)** With `whisper-cli` and the selected model
...
- **MIGRATED** **V2-03 READY** Select local STT, make the local engine unavailable, and
      dictate. Expected: no cloud upload/fallback; actionable local prerequisite.
- **MIGRATED** **V2-04 CONDITIONAL (macOS/Windows)** Deny microphone permission. Expected:
      explicit OS recovery instructions. Repeat with no microphone device.
- **MIGRATED** **V2-05 READY (scope closed)** Stage 2 uses batch whisper.cpp. Live/tentative
      sidecar STT is explicitly deferred and is not an exit requirement for this change.
- **MIGRATED** **V2-06 READY** Hold the voice hotkey, release it while microphone permission
...
- **MIGRATED** **V3-01 CONDITIONAL (macOS)** Native preview speaks audibly at selected
      voice/rate. Stop interrupts immediately.
- **MIGRATED** **V4-01 CONDITIONAL (Windows)** SAPI preview speaks audibly and Stop
      interrupts immediately.
- **MIGRATED** **V5-01 CONDITIONAL** Kokoro dependencies/model install, offline synthesis,
      selected Kokoro voice, and playback all work without an API key.
- **MIGRATED** **V5-02 READY** Force Kokoro failure after selecting a Kokoro-only voice.
...
- **MIGRATED** **V6-01 READY** Enable read-aloud for Claude, Codex, and Cursor. Expected:
      only the harness-authored `Spoken:` block is read; code/tables/logs and the
      `Displayed:` block are not spoken.
- **MIGRATED** **V6-02 READY** Repeat V6-01 for OpenRouter and NanoGPT. The OpenCode prompt
...
- **MIGRATED** **V6-03 READY** Use a reply without `Spoken:`. Expected: deterministic
      non-LLM fallback text, no extra model request, and persisted resolved speech.
- **MIGRATED** **V7-01 READY** Click Play on an existing message. Expected: chat/sub-chat/
...
- **MIGRATED** **V7-02 READY** Start a second utterance, start a new run, and press visible
...
- **MIGRATED** **V7-03 READY** Toggle global and per-chat read-aloud. Expected: per-chat
      override wins, an inheritance/reset path is understandable, and history does
      not auto-play.
- **MIGRATED** **V8-01 READY** Select tiny, base, and small in Settings. Expected: each
...
- **MIGRATED** **V9-01 CONDITIONAL (packaged macOS/Windows)** Verify usage strings,
...
- **MIGRATED** **U1-01 READY** Migrate a seeded pre-Stage-2 DB. Expected: samples, cycles,
      alerts, provider states, and daemon status tables appear without data loss.
- **MIGRATED** **U1-02 READY** Insert daemon/app/reconcile duplicates. Expected: stable
      dedupe keys prevent double count; token-only samples persist with unknown cost.
- **MIGRATED** **U1-03 READY** Upsert estimated after exact/provider-reported data.
...
- **MIGRATED** **U2-01 READY** Run fake providers in app and daemon modes. Expected:
      equivalent normalized samples and provider states.
- **MIGRATED** **U2-02 READY** Hang a provider request. Expected: 15-second timeout,
...
- **MIGRATED** **U3-01 READY (macOS)** Install/start daemon, close Flapstack, wait one
...
- **MIGRATED** **U3-02 READY (macOS)** Disable/uninstall daemon. Expected: process exits,
      status stops advancing, and no duplicate/orphan process remains.
- **MIGRATED** **U3-03 CONDITIONAL (Windows/Linux)** Install/disable the Windows per-user
...
- **MIGRATED** **U3-04 READY (macOS Electron)** Through a temporary isolated no-TTY
      ...
      or error text.
- **MIGRATED** **U4-01 READY** Keep app reads open during daemon writes. Expected: WAL/
      busy-timeout/retry prevents silent loss. Repeat a forced lock and corruption
      case and verify visible failure.
- **MIGRATED** **U5-01 READY** Stop daemon for a gap, relaunch app, and press Refresh now.
...
- **MIGRATED** **U10-01 READY** Cross a quota threshold with app closed. Expected: exactly
      one Discord event; app later shows the persisted delivery.
- **MIGRATED** **U10-02 READY** Return below threshold then cross again. Expected: re-arm
      and one new alert.
- **MIGRATED** **U10-03 READY** Force Discord 500 then recover. Expected: failed event is
      visible, threshold remains armed, next daemon tick retries, success then
      disarms it.
- **MIGRATED** **U10-04 CONDITIONAL (OpenRouter/NanoGPT key)** Verify per-run spend alerts.
...
- **MIGRATED** **U6-03 CONDITIONAL (personal subscriptions)** With local Codex and Claude
...
- **MIGRATED** **U7-01 CONDITIONAL (Cursor logged in)** Auto-detect local Cursor token and
...
- **MIGRATED** **U7-02 CONDITIONAL (Cursor logged in)** Verify source 1 plan info, credit
...
- **MIGRATED** **U8-01 CONDITIONAL (OpenRouter key)** Poll key limits/balance and label the
...
- **MIGRATED** **U8-02 CONDITIONAL (OpenRouter run)** Persist run tokens and estimated
...
- **MIGRATED** **U8-03 READY** A 404/unavailable generation remains an honest gap; no fake
      zero-dollar exact sample is written. It becomes terminal instead of retrying
      every poll; manual retry clears that state.
- **MIGRATED** **U8-04 READY** Render an OpenRouter key-limit sample with percentage absent.
      Expected: `usd-micros` quota values display as dollars (for example `$5 / $20`),
      never as raw `5,000,000 / 20,000,000` integers.
- **MIGRATED** **U9-01 CONDITIONAL (NanoGPT key)** Refresh model pricing and capture run
...
- **MIGRATED** **U9-02 READY** UI says NanoGPT is run-usage-only unless a current verified
      account-history endpoint exists.

### Dashboard/settings

- **MIGRATED** **U11-01 READY** Open Settings → Usage. Expected: current summary cards,
...
- **MIGRATED** **U11-02 READY** Configure cadence, provider toggles, thresholds, keys,
      Discord webhook, daemon state, and Refresh now without editing files.
- **MIGRATED** **U11-03 READY** Historical quota, cost, and token charts render in the
...
- **MIGRATED** **U11-04 READY** On a platform without daemon support, the install action is
      hidden/disabled with an honest explanation before click.
- **MIGRATED** **U11-05 READY** Query/provider/refresh failure is visible and cannot look
      like a legitimate empty or zero state.

## 4. Track D - Cursor harness

- **MIGRATED** **D0-01 READY** `cursor-agent --version`, `--help`, model listing, and login
      status match the adapter assumptions. Record CLI version.
- **MIGRATED** **D0-02 READY** Model output headers such as `Models:` and `Available models`
...
- **MIGRATED** **D1-01 READY** Cursor appears as a first-class provider with teal chip in
      new chat, chat header, sidebar, and run history.
- **MIGRATED** **D2-01 CONDITIONAL (Cursor login)** Start a read-only Cursor chat with
...
- **MIGRATED** **D2-02 READY** A completed-only reasoning event creates one complete
...
- **MIGRATED** **D2-03 READY** Structured error/result failure marks the run failed even if
...
- **MIGRATED** **D3-01 CONDITIONAL** Connected/not-logged-in states are accurate; Connect
...
- **MIGRATED** **D3-02 CONDITIONAL (Cursor API key)** Configure a Cursor API key without
...
- **MIGRATED** **D4-01 READY** Before launch, each permission mode shows applied Cursor
...
- **MIGRATED** **D4-02 CONDITIONAL** Ask-before-edits prompts for a tool action; read-only
      blocks edits; full access visibly warns before launch.
- **MIGRATED** **D5-01 READY** Select a live Cursor model, create a chat, continue an
      existing chat with Cursor, and verify exact provider/model persistence.
- **MIGRATED** **D5-02 READY** Images are rejected before run with an honest unsupported
...
- **MIGRATED** **E1-01 READY** OpenRouter and NanoGPT appear in normal new-chat provider
      selection. Selected exact catalog model persists on chat/sub-chat/run.
- **MIGRATED** **E2-01 READY** Sidecar starts with isolated config and generated local
...
- **MIGRATED** **E2-02 READY** Prompt uses `/prompt_async` after SSE subscription. Expected:
...
- **MIGRATED** **E3-01 CONDITIONAL** Provider keys are stored securely; generated config
      references environment variables and never contains key material.
- **MIGRATED** **E3-02 READY** Missing key/model/binary stops before provider work and
      produces an actionable state. A missing key should not create a misleading run.
- **MIGRATED** **E4-01 CONDITIONAL (OpenRouter key)** Run a minimal completion. Expected:
      valid text stream, `reasoning-start → deltas → reasoning-end`, persisted text/
      reasoning, and success status.
- **MIGRATED** **E4-02 CONDITIONAL (NanoGPT key)** Repeat E4-01 using NanoGPT.
- **MIGRATED** **E5-01 CONDITIONAL** Trigger shell/edit/web permission. Expected: approval
      UI shows exact command plus requested patterns/paths before **Allow once**,
      **Always allow**, or **Deny**.
- **MIGRATED** **E5-02 CONDITIONAL** Deny and allow each request. Expected: provider tool
      loop continues correctly; decision and request scope persist in run metadata.
- **MIGRATED** **E5-03 READY** No approval handler defaults to rejection, never silent allow.
- **MIGRATED** **E6-01 CONDITIONAL** Successful/failed/cancelled runs persist status,
...
- **MIGRATED** **E6-02 READY** Force usage persistence failure after provider completion.
      Expected: telemetry error cannot leave run/sub-chat stuck `running`.
- **MIGRATED** **E6-03 CONDITIONAL (OpenRouter key)** Run a multi-step tool loop. Expected:
...
- **MIGRATED** **E7-01 READY** Settings key status, model refresh/cache/seed distinction,
      default selection, provider chips, and new-chat flow agree.
- **MIGRATED** **E7-02 CONDITIONAL (provider catalogs)** Refresh OpenRouter and NanoGPT.
...
- **MIGRATED** **E7-03 READY** OpenCode prompts receive the Voice `Spoken:`/`Displayed:`
      read-aloud instruction. Verify both providers live before checking this row.

## 6. Track T - reasoning-output parity

- **MIGRATED** **T1-01 READY** Shared fixtures normalize visible reasoning, summary,
...
- **MIGRATED** **T2-01 READY** Three deltas visibly grow one reasoning panel and final does
...
- **MIGRATED** **T2-02 READY** Scoped/in-chat search finds standard reasoning and legacy
      visible reasoning tools while excluding hidden tool inputs.
- **MIGRATED** **T3-01 CONDITIONAL (Claude)** Live Claude visible reasoning streams and
      final-only/backfill behavior match fixtures.
- **MIGRATED** **T4-01 CONDITIONAL (Codex)** ACP visible thought renders; token-only usage
      is labeled reasoning tokens; encrypted content stays opaque.
- **MIGRATED** **T5-01 CONDITIONAL (Cursor)** Live Cursor reasoning appears before final
      answer when emitted; no-reasoning reply remains normal.
- **MIGRATED** **T6-01 CONDITIONAL (OpenRouter/NanoGPT)** Provider visible reasoning and
      legacy `reasoning_content` use the same panel and valid stream lifecycle.
- **MIGRATED** **T7-01 CONDITIONAL** Complete the platform/provider manual matrix in
...
- **MIGRATED** Voice matrix passed on macOS and Windows for required rows.
- **MIGRATED** Usage daemon/provider/alert matrix passed with app closed and reopened.
- **MIGRATED** Cursor, OpenRouter, and NanoGPT live UI paths passed where credentials exist.
- **MIGRATED** Reasoning matrix passed through persisted UI, not fixtures alone.
- **MIGRATED** Track C deep-count and discoverability cases passed.
- **MIGRATED** `npm run check`, strict TypeScript gate, and strict OpenSpec validation pass.
- **MIGRATED** Status docs and OpenSpec tasks match actual evidence and known limitations.
