...
import { MobileSnapshotProjector, type MobileProjectedKind } from "./projector"
...
export class MobileEventService {
  ...
  constructor(
    ...
    this.projector = new MobileSnapshotProjector(options.database)
  }
  ...
  ingestInvalidation(event: ProductMcpRendererInvalidation): void {
    ...
    for (const grant of this.options.grants.listActiveGrants()) {
      ...
      for (const ref of refs) {
        ...
        if (
          !this.projector.resourceExists(ref.kind, ref.id) &&
          this.wasProjected(grant.grantId, ref.kind, ref.id)
        ) {
          this.appendEvent(grant.grantId, ref)
        }
      }
  ...
  handleWebSocket = (transport: MobileBridgeTransportSession): void => {
    ...
    const unsubscribeMessage = transport.onMessage((bytes) => {
      ...
      try {
        const request = mobileSnapshotPageRequestSchema.parse(parseJson(bytes))
  ...
  private decodePageCursor(value: string): PageCursor {
    ...
    if (
      ...
      !["project", "task", "chat", "run", "orchestration", "automation"].includes(parsed.kind) ||
...
function isProjectedKind(value: string): value is MobileProjectedKind {
  return ["project", "task", "chat", "run", "orchestration", "automation"].includes(value)
}
