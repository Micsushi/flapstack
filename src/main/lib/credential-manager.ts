  ...
  markSourceNeedsReauth(source: LoadedSource, errorMessage: string): void {
    try {
      ...
      if (config) {
        ...
        debug(
          `[SourceCredentialManager] Marked ${source.config.slug} as needing re-auth: ${errorMessage}`,
        )
      }
    } catch (error) {
      debug(
        `[SourceCredentialManager] Failed to mark ${source.config.slug} as needing re-auth:`,
        error,
      )
    }
  }
