...
export async function listNativeSkillMentions(
  ...
  if (input.subChatId) {
    const row = database.select({
      projectId: chats.projectId, taskId: chats.taskId, harness: subChats.harness,
      chatHarness: chats.harness, worktreePath: subChats.worktreePath, chatWorktreePath: chats.worktreePath,
    }).from(subChats).innerJoin(chats, eq(chats.id, subChats.chatId))
      .where(eq(subChats.id, input.subChatId)).get()
  ...
  if (taskId) {
    const task = database.select({ projectId: tasks.projectId }).from(tasks).where(eq(tasks.id, taskId)).get()
    if (!task || !projectId || task.projectId !== projectId) throw new Error("Task does not belong to the requested project")
  }
  if (projectId) {
    const project = database.select({ path: projects.path }).from(projects).where(eq(projects.id, projectId)).get()
  ...
  const state = await resolveExtensionInventoryState(database, { ...options, cwd, projectId, taskId, harness })
  const native = state.filter(({ extension }) => extension.kind === "skill")
    .sort((a, b) => Number(b.extension.source === "project") - Number(a.extension.source === "project")
      || Number(a.extension.capabilities.discovery === "compatibility") - Number(b.extension.capabilities.discovery === "compatibility"))
  const seen = new Set<string>()
  const skills = native.filter(({ extension, resolved }) => {
