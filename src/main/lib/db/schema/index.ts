...
export const discussionTopics = sqliteTable(
  ...
  (table) => [
    index("discussion_topics_scope_idx").on(
      table.projectId,
      table.chatId,
      table.hostId,
      table.updatedAt,
...
export const discussionRevisions = sqliteTable(
...
export const workspaceEdits = sqliteTable(
  "workspace_edits",
  {
    ...
    kind: text("kind", { enum: ["save", "create", "remove", "rename"] })
      .notNull()
      .default("save"),
    fileMode: integer("file_mode").notNull().default(384),
    previousRelativePath: text("previous_relative_path"),
    state: text("state", {
