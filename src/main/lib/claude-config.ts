 ...
 * Supports legacy (~/.flapstack/worktrees/{projectId}/{chatId}/) and
 * new format (~/.flapstack/worktrees/{projectName}/{worktreeFolder}/).
 *
...
export function resolveProjectPathFromWorktree(pathToResolve: string): string | null {
  const worktreeMarker = path.join(".flapstack", "worktrees")
  ...
  try {
    ...
    // Path format: /Users/.../.flapstack/worktrees/{projectSlug}/{worktreeFolder}
    const worktreeBase = path.join(os.homedir(), ".flapstack", "worktrees")
