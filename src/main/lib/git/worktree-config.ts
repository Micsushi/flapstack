...
export type WorktreeConfigSource = "custom" | "cursor" | "flapstack" | null
...
const FLAPSTACK_CONFIG_PATH = ".flapstack/worktree.json"
 ...
 * Priority: custom path > .cursor/worktrees.json > .flapstack/worktree.json
 */
export async function detectWorktreeConfig(
  ...
  // 3. Check .flapstack/worktree.json
  const flapstackPath = join(projectPath, FLAPSTACK_CONFIG_PATH)
  if (await fileExists(flapstackPath)) {
    const config = await readJsonFile<WorktreeConfig>(flapstackPath)
    if (config) {
      return { config, path: flapstackPath, source: "flapstack" }
    }
  }
...
export async function getAvailableConfigPaths(projectPath: string): Promise<{
  ...
  flapstack: { exists: boolean; path: string }
}> {
  const cursorPath = join(projectPath, CURSOR_CONFIG_PATH)
  const flapstackPath = join(projectPath, FLAPSTACK_CONFIG_PATH)
  ...
  return {
    ...
    flapstack: {
      exists: await fileExists(flapstackPath),
      path: flapstackPath,
    },
  }
}
...
export async function saveWorktreeConfig(
  ...
  target: "cursor" | "flapstack" | string = "flapstack",
): Promise<{ success: boolean; path: string; error?: string }> {
  let targetPath: string

  if (target === "cursor") {
    targetPath = join(projectPath, CURSOR_CONFIG_PATH)
  } else if (target === "flapstack") {
    targetPath = join(projectPath, FLAPSTACK_CONFIG_PATH)
  } else {
    // Custom path
