...
export class MobileBridgeService {
  ...
  listInterfaces(): MobileNetworkInterface[] {
    return this.options.network.list().filter((entry) => {
      try {
        validateMobileBridgeBindAddress(entry.address, [entry])
        return true
      } catch {
        return false
      }
      ...
      try {
        ...
        try {
          ...
          failure = new AggregateError(
            [error, cleanupError],
            "Mobile bridge startup and listener cleanup both failed.",
          )
        }
  ...
  private async startLocked(address: string | null, port: number): Promise<void> {
    ...
    // Subscription takes its own current-network baseline. Validate a fresh
    // snapshot only after cleanup owns that subscription, so an interface change
    // in the startup-to-subscribe gap cannot become an accepted unsafe baseline.
    validateMobileBridgeBindAddress(address, this.options.network.list())
  ...
  private handleNetworkChange(interfaces: MobileNetworkInterface[]): Promise<void> {
    return this.enqueue(async () => {
      ...
      try {
        ...
        let failure = "network-changed"
        try {
          await this.stopLocked("network-changed")
        } catch (cleanupError) {
          failure = safeFailure(cleanupError)
        }
        this.settings = { ...this.settings, enabled: false }
        this.state = "faulted"
  ...
  private handleTransportFailure(error: Error): Promise<void> {
    return this.enqueue(async () => {
      ...
      try {
        ...
        failure = new AggregateError(
          [error, cleanupError],
          "Mobile bridge transport failure and listener cleanup both failed.",
        )
      }
      this.settings = { ...this.settings, enabled: false }
      this.state = "faulted"
