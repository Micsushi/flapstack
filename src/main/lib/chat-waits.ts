...
export type ActiveAgentChatWait = {
  id: string
  chatId: string
...
export function registerChatWait(
  ...
  return { ok: true, id, created: true, state: "waiting", targetChatIds }
}
...
export function listActiveAgentChatWaits(database: Database.Database): ActiveAgentChatWait[] {
  const rows = database
    .prepare(
      `SELECT id, waiter_chat_id, target_chat_ids, status, created_at
       FROM chat_waits WHERE status IN ('waiting','resuming') ORDER BY created_at, id`,
    )
    .all() as Array<{
    id: string
