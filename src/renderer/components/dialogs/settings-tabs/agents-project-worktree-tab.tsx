...
function ProjectDetail({ projectId }: { projectId: string }) {
  ...
  // Local state
  const [saveTarget, setSaveTarget] = useState<"cursor" | "flapstack">("flapstack")
  ...
  useEffect(() => {
    if (configData) {
      const newSaveTarget = configData.source === "cursor" ? "cursor" : "flapstack"
          ...
          <div className="bg-background rounded-lg border border-border overflow-hidden">
            <div className="flex items-center justify-between p-4">
              ...
              <Select
                value={saveTarget}
                onValueChange={(v) => {
                  setSaveTarget(v as "cursor" | "flapstack")
                  pendingSaveRef.current = true
                }}
              >
                <SelectTrigger className="w-auto px-3">
                  <span className="text-sm font-mono">
                    {saveTarget === "cursor"
                      ...
                      : ".flapstack/worktree.json"}
                  </span>
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="flapstack">.flapstack/worktree.json</SelectItem>
