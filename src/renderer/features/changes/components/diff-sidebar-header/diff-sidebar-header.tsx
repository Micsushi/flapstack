...
export const DiffSidebarHeader = memo(function DiffSidebarHeader({
  ...
  const handleForcePush = () => {
    if (
      window.confirm("Are you sure you want to force push? This will overwrite the remote branch.")
    ) {
      ...
      >
                  ...
                  {displayAction.isPending ? (
                    <>
                      ...
                      {displayAction.pendingLabel && (
                        <span className="mr-0.5 truncate">{displayAction.pendingLabel}</span>
                      )}
                      {displayAction.badge && (
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-52">
                ...
                {((hasUpstream &&
                  ...
                  onCreatePr &&
                  !isDefaultBranch &&
                  primaryAction.label !== "Create PR") ||
                  (hasUpstream && !pr && onCreatePrWithAI && !isDefaultBranch) ||
                  pr ||
                  (hasPrNumber && isPrOpen && onMergePr)) && <DropdownMenuSeparator />}
                ...
                {hasPrNumber && isPrOpen && onMergePr && !hasMergeConflicts && (
                  <DropdownMenuItem onClick={onMergePr} disabled={isMergingPr} className="text-xs">
                    <GitMerge className="mr-2 size-3.5" />
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-48">
            ...
            {!showReviewButton && diffStats.hasChanges && onReview && (
              <DropdownMenuItem onClick={onReview} disabled={isReviewing} className="text-xs">
