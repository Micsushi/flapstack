...
import { copyChat } from "../lib/export-chat"
...
import {
  ...
  clearAgentSubChatStoreProjection,
...
const CHAT_LAYOUT = {
  ...
  // Leave breathing room between floating prompts and the conversation header divider.
  stickyTopSidebarOpen: "top-2",
...
const claudeModels = [
  { id: "opus", name: "Opus auto" },
  { id: "sonnet", name: "Sonnet auto" },
...
function CopyButton({ onCopy, isMobile = false }: { onCopy: () => void; isMobile?: boolean }) {
...
const ScrollToBottomButton = memo(function ScrollToBottomButton({
  ...
  useEffect(() => {
    ...
    let lastIsVisible: boolean | null = null

    const getShouldShow = () => {
      const scrollableBuffer = 4
      const isScrollable = container.scrollHeight > container.clientHeight + scrollableBuffer
      const isPastLatestThreshold =
        getDistanceFromScrollBottom(container) > getScrollToLatestThreshold(container)
      return isScrollable && isPastLatestThreshold
    }
    ...
    const checkVisibility = () => {
      ...
      rafId = requestAnimationFrame(() => {
        ...
        if (lastIsVisible !== shouldShow) {
          lastIsVisible = shouldShow
        ...
        <Tooltip delayDuration={300}>
          <TooltipTrigger asChild>
            <motion.div
              ...
              className="pointer-events-none absolute inset-x-0 z-20 px-2 transition-[bottom] duration-200"
              style={{
                bottom: hasStackedCards
                  ? "calc(var(--chat-input-height, 4rem) + 4.75rem)"
                  : "calc(var(--chat-input-height, 4rem) + 0.75rem)",
              }}
            >
              <div className="mx-auto flex max-w-2xl justify-center">
                <button
                  ...
                  className={cn(
                    "pointer-events-auto flex w-fit max-w-[calc(100vw-2rem)] items-center gap-2 rounded-full bg-background border border-border px-3 py-2 text-sm shadow-md hover:bg-accent active:scale-[0.97] transition-colors duration-200",
                  )}
                  aria-label="Scroll to bottom"
                >
                  <ArrowDown className="h-4 w-4 text-muted-foreground" />
                  <span className="whitespace-nowrap">
                    {hasUnreadUpdate ? "New message ready" : "Latest messages"}
                  </span>
                </button>
              </div>
            </motion.div>
...
const ChatViewInner = memo(function ChatViewInner({
  ...
  isActive?: boolean
  isSplitPane?: boolean
  acceptsGlobalShortcuts?: boolean
  ...
  const messageVirtualizerRef = useRef<MessageVirtualizerHandle>(null)
  const [hasUnreadBottomUpdate, setHasUnreadBottomUpdate] = useState(false)
  ...
  const saveScrollPosition = useCallback(() => {
    ...
    scrollPositionCache.set(subChatId, position)
    writeWorkbenchScrollPosition(localStorage, parentChatId, subChatId, position)
  ...
  const scheduleScrollPositionSave = useCallback(() => {
    if (pendingScrollSaveFrameRef.current !== null) return
  ...
  const isAtBottom = useCallback(() => {
    ...
    return getDistanceFromScrollBottom(container) <= threshold
  }, [])
  ...
  const handleScroll = useCallback(() => {
    ...
    if (shouldAutoScrollRef.current) {
      setHasUnreadBottomUpdate(false)
    }
  }, [isAtBottom, scheduleScrollPositionSave])
  ...
  const localUtils = trpc.useUtils()
  ...
  const handleRenameSubChat = useCallback(
    async (newName: string) => {
      ...
      subChatStore.getState().updateSubChatName(subChatId, newName)
      localUtils.chats.list.setData({}, (old) =>
        old?.map((chat) => (chat.id === parentChatId ? { ...chat, name: newName } : chat)),
      )
      localUtils.chats.get.setData({ id: parentChatId }, (old) =>
        old
          ? {
              ...old,
              name: newName,
              subChats: old.subChats.map((subChat) =>
                subChat.id === subChatId ? { ...subChat, name: newName } : subChat,
              ),
            }
          : old,
      )
      ...
      try {
        ...
        subChatStore.getState().updateSubChatName(subChatId, subChatNameRef.current || "New Chat")
        await Promise.all([
          localUtils.chats.list.invalidate(),
          localUtils.chats.get.invalidate({ id: parentChatId }),
        ])
      }
    },
        ...
        .then(async () => {
          try {
            ...
            if (latestRequestedModeRef.current === mode) {
              ...
              subChatStore.getState().updateSubChatMode(subChatId, revertedMode)
            }
  ...
  useEffect(() => {
    if (subChatId) {
      ...
      subChatStore.getState().updateSubChatMode(subChatId, mode)
    }
  }, [subChatId])
  ...
  const {
    images,
    files,
    handleAddAttachments: handleAddPreviewAttachments,
    removeImage,
    removeFile,
    clearAll,
    isUploading,
  ...
  const attachmentUtils = trpc.useUtils()
  const attachmentImportMutation = trpc.attachments.importFile.useMutation({
    onSuccess: () => {
      attachmentUtils.attachments.listByChat.invalidate({ chatId: parentChatId })
    },
    onError: (error) => {
      toast.error(error.message || "Failed to persist attachment")
    },
  })
  const attachmentTextMutation = trpc.attachments.createText.useMutation({
    onSuccess: () => {
      attachmentUtils.attachments.listByChat.invalidate({ chatId: parentChatId })
    },
    onError: (error) => {
      toast.error(error.message || "Failed to persist pasted text")
    },
  })
    ...
    async (inputFiles: File[]) => {
      for (const file of inputFiles) {
        if (file.size > MAX_ATTACHMENT_BYTES) {
          toast.error(`Attachment exceeds the ${MAX_ATTACHMENT_BYTES}-byte size limit`)
          continue
        }
        ...
        attachmentImportMutation.mutate({
          ...
          mimeType: file.type.startsWith("image/")
            ? file.type
            : normalizeNonImageAttachmentMimeType(file.type),
        })
      }
    },
  ...
  const handleAddAttachments = useCallback(
    async (inputFiles: File[]) => {
      const boundedFiles = inputFiles.filter((file) => {
        if (file.size <= MAX_ATTACHMENT_BYTES) return true
        toast.error(`Attachment exceeds the ${MAX_ATTACHMENT_BYTES}-byte size limit`)
        return false
      })
      if (boundedFiles.length === 0) return
      await handleAddPreviewAttachments(boundedFiles)
      await persistAttachments(boundedFiles)
    },
  ...
  const {
    pastedTexts,
    addPastedText: addPastedTextFile,
    addChatHistoryFile,
    removePastedText,
    clearPastedTexts,
    pastedTextsRef,
  ...
  const addPastedText = useCallback(
    async (text: string) => {
      await addPastedTextFile(text)
      attachmentTextMutation.mutate({
        chatId: parentChatId,
        taskId: taskId ?? undefined,
        kind: "pasted-text",
    ...
    [addPastedTextFile, attachmentTextMutation, parentChatId, taskId],
  )
  ...
  useEffect(() => {
    if (!isActive || !acceptsGlobalShortcuts) return
  ...
  const handleApprovePlan = useCallback(() => {
    ...
    subChatStore.getState().updateSubChatMode(subChatId, "write")
    ...
    async (
      ...
      const userMsgImages: UploadedImage[] = (userMsg.parts || [])
        ...
        .map((p: any) => ({
          ...
          url:
            p.data.url ||
            (p.data.base64Data && p.data.mediaType
  ...
  const handleForkFromMessage = useCallback(
    async (messageId: string) => {
      ...
      try {
        ...
        await Promise.all([
          localUtils.chats.list.invalidate({}),
          utils.agents.getAgentChat.invalidate({ chatId: newChat.id }),
        ])
        ...
        // Open and select the new top-level chat. Its canonical conversation
        // will hydrate from the newly created one-to-one internal row.
        appStore.set(openAgentChatIdsAtom, (current) =>
          current.includes(newChat.id) ? current : [...current, newChat.id],
        )
        appStore.set(selectedAgentChatIdAtom, newChat.id)
      } catch (error) {
    ...
    [isStreaming, subChatId, utils, localUtils],
  )
  ...
  useFocusInputOnEnter(editorRef, isActive && acceptsGlobalShortcuts)

  // Keyboard shortcut: Cmd+Esc to toggle focus/blur (without stopping generation)
  useToggleFocusOnCmdEsc(editorRef, isActive && acceptsGlobalShortcuts)
  ...
  useLayoutEffect(() => {
    ...
    const savedPosition =
      readWorkbenchScrollPosition(localStorage, parentChatId, subChatId) ??
      scrollPositionCache.get(subChatId)
  ...
  }, [subChatId, isVisiblePane, parentChatId])
  ...
  const previousMessageStateRef = useRef({ count: messages.length, status })

  useEffect(() => {
    if (!isVisiblePane || !scrollInitializedRef.current) {
    ...
    const streamCompleted = previous.status === "streaming" && status === "ready"

    const container = chatContainerRef.current
    const hasUnreadDistance =
      !!container && getDistanceFromScrollBottom(container) > getScrollToLatestThreshold(container)

    if (
      (messageWasAdded || streamCompleted) &&
      !shouldAutoScrollRef.current &&
      !isAtBottom() &&
      hasUnreadDistance
    ) {
      setHasUnreadBottomUpdate(true)
    }
  ...
  const handleSend = useCallback(
    async (inputValueOverride?: string) => {
      ...
      // Voice sends can finish after this chat is no longer mounted. Prefer the
      // payload captured when Send was clicked over navigation-sensitive editor state.
      const inputValue = inputValueOverride ?? editorRef.current?.getValue() ?? ""
  ...
  useEffect(() => {
    ...
    window.addEventListener("keydown", handleKeyDown)
    return () => window.removeEventListener("keydown", handleKeyDown)
  }, [acceptsGlobalShortcuts, isActive, hasUnapprovedPlan, isStreaming, handleApprovePlan])
  ...
  useEffect(() => {
    ...
    window.addEventListener("keydown", handleKeyDown)
    return () => window.removeEventListener("keydown", handleKeyDown)
  }, [acceptsGlobalShortcuts, isActive, scrollToBottom])
  ...
  useEffect(() => {
    if (!isActive || !acceptsGlobalShortcuts || !currentSearchMatch) return
  ...
  }, [acceptsGlobalShortcuts, currentSearchMatch, isActive])
  ...
  const jumpToTranscriptMarker = useCallback((marker: TranscriptMarker) => {
    ...
    const target = Array.from(
      ...
      (element) =>
        element.dataset.userMessageId === marker.id ||
        element.dataset.assistantMessageId === marker.id,
    )
    if (!target) return
  ...
  // Continue with a different provider through the durable top-level Chat authority.
  const isContinuingRef = useRef(false)
  const providerContinuationRequestRef = useRef<string | null>(null)
  const handleContinueWithProvider = useCallback(
    async (targetProvider: AgentProviderId, targetModel: string) => {
      ...
      try {
        const preference =
          targetProvider === "claude-code"
            ...
            : targetProvider === "codex"
              ? "codex"
              : "flapstack-native"
        const requestId = providerContinuationRequestRef.current ?? crypto.randomUUID()
        providerContinuationRequestRef.current = requestId
        const preview = await trpcClient.chats.previewRuntimeContinuation.query({
        ...
        if (!confirmed) {
          providerContinuationRequestRef.current = null
          return
        }
        const created = await trpcClient.chats.continueWithRuntime.mutate({
          sourceChatId: parentChatId,
          targetHarness: targetProvider,
          targetModel,
          preference,
        ...
        await localUtils.chats.list.invalidate({})
        appStore.set(openAgentChatIdsAtom, (current) =>
          current.includes(created.chatId) ? current : [...current, created.chatId],
        )
        appStore.set(selectedAgentChatIdAtom, created.chatId)
        providerContinuationRequestRef.current = null
        toast.success(`Continued in a new ${targetProvider} Chat`)
      } catch (error) {
        console.error("[handleContinueWithProvider] Error:", error)
        toast.error(error instanceof Error ? error.message : "Failed to continue with provider")
      } finally {
        isContinuingRef.current = false
      }
    },
    [isStreaming, localUtils.chats.list, messages, parentChatId],
  )
  const isDelegatingRef = useRef(false)
  const handleDelegateWithProvider = useCallback(
    async (targetProvider: AgentProviderId, targetModel: string) => {
      if (isStreaming || isDelegatingRef.current || !messages?.length) return
      const objective = window
        .prompt("What bounded task should the child provider complete?")
        ?.trim()
      if (!objective) return
      isDelegatingRef.current = true
      try {
        const preference: "claude-code" | "codex" | "flapstack-native" =
          targetProvider === "claude-code"
            ? "claude-code"
            : targetProvider === "codex"
              ? "codex"
        ...
        const preview = await trpcClient.chats.previewRuntimeDelegation.query(request)
        ...
        if (!confirmed) return
        const created = await trpcClient.chats.delegateToRuntime.mutate({
          ...request,
          confirmedPreviewDigest: preview.digest,
        })
        await localUtils.chats.list.invalidate({})
        appStore.set(openAgentChatIdsAtom, (current) =>
          current.includes(created.childChatId) ? current : [...current, created.childChatId],
        )
        appStore.set(selectedAgentChatIdAtom, created.childChatId)
        toast.success(`Delegated to a new ${targetProvider} child Chat`)
      } catch (error) {
        console.error("[handleDelegateWithProvider] Error:", error)
...
// Each top-level Chat owns an isolated compatibility store. The global store is
// only a projection of the active pane for legacy shared navigation.
export function ChatView(props: ChatViewProps) {
  return (
    <AgentSubChatStoreScope chatId={props.chatId}>
      <ChatViewScoped {...props} />
    </AgentSubChatStoreScope>
  )
}

// Chat View wrapper - handles loading and creates chat object
function ChatViewScoped({
  chatId,
  isSidebarOpen,
  onToggleSidebar,
  selectedTeamName,
  ...
  const subChatStore = useAgentSubChatStoreApi()
  const acceptsTopLevelShortcuts = workbenchActive ?? true
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (!acceptsTopLevelShortcuts) return
  ...
  const { activeSubChatId, openSubChatIds, pinnedSubChatIds, allSubChats, splitPaneIds } =
    scopedSubChatState
  useEffect(() => {
    if (workbenchActive === false) return
    projectAgentSubChatStore(subChatStore)
    return () => clearAgentSubChatStoreProjection(subChatStore)
  }, [scopedSubChatState, subChatStore, workbenchActive])
  ...
  const activeTargetWorktreePathAtom = useMemo(
    () => selectedTargetWorktreePathAtomFamily(activeSubChatId ?? ""),
    [activeSubChatId],
  )
  ...
  const handleCreatePr = useCallback(async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Failed to prepare PR request", {
        position: "top-center",
      })
      setIsCreatingPr(false)
    }
...
Make sure to preserve all functionality from both branches when resolving conflicts.`
  ...
  useEffect(() => {
    ...
    const freshState = subChatStore.getState()
    ...
    const dbSubChats: SubChatMeta[] = agentSubChats.map((sc) => {
      ...
      const createdAt =
        typeof sc.created_at === "string" ? sc.created_at : sc.created_at?.toISOString()
      const updatedAt =
        typeof sc.updated_at === "string" ? sc.updated_at : sc.updated_at?.toISOString()
      return {
        id: sc.id,
  ...
  const inferProviderFromMessages = useCallback(
    (subChatId?: string): AgentProviderId => {
      ...
      ) as { harness?: string | null; model?: string | null; messages?: any } | undefined
      const chatHarness = (agentChat as any)?.harness
      ...
      const persistedModel = subChat?.model || chatModel
      if (typeof persistedModel === "string") {
        const normalizedModel = persistedModel.toLowerCase()
  ...
  const codexMcpSessionData = useMemo(() => {
    ...
    for (const server of effectiveServers.values()) {
      ...
      for (const tool of Array.isArray(server.tools) ? server.tools : []) {
        const toolName =
          typeof tool === "string" ? tool : typeof tool?.name === "string" ? tool.name : null
  ...
  }, [acceptsTopLevelShortcuts, activeSubChatProvider, codexMcpSessionData, setSessionInfo])
      ...
      if (isRemoteChat && chatSandboxUrl) {
        ...
        if (chatProvider === "local") {
          ...
          transport = new ACPChatTransport({
            ...
            projectPath,
            projectId:
              (agentChat as any)?.project?.id ?? (agentChat as any)?.projectId ?? undefined,
            mode: subChatMode,
            provider: "codex",
          })
      ...
      const newChat = new Chat<any>({
        ...
        onError: (error: Error) => {
          ...
          const failedSubChatName =
            ...
            agentChat?.name ||
            "Agent"
          notifyAgentError(`${failedSubChatName}: ${error.message || "Agent request failed"}`, {
            chatId,
            subChatId,
        ...
        onFinish: (event: any) => {
          ...
          const currentActiveSubChatId = subChatStore.getState().activeSubChatId
          ...
          if (!wasManuallyAborted) {
            const finishedSubChatName =
              subChatStore.getState().allSubChats.find((chat) => chat.id === subChatId)?.name ||
              subChat?.name ||
              agentChat?.name ||
              "Agent"
            notifyAgentComplete(finishedSubChatName, {
              chatId,
              subChatId,
              parentChatName: agentChat?.name || undefined,
            })
    ...
    if (newSubChatTransport) {
      ...
      const newChat = new Chat<any>({
        ...
        onError: (error: Error) => {
          ...
          const failedSubChatName =
            subChatStore.getState().allSubChats.find((chat) => chat.id === newId)?.name ||
            "New Chat"
          notifyAgentError(`${failedSubChatName}: ${error.message || "Agent request failed"}`, {
            chatId,
            subChatId: newId,
  ...
  useEffect(() => {
    ...
    window.addEventListener("keydown", handleKeyDown)
    return () => window.removeEventListener("keydown", handleKeyDown)
  }, [acceptsTopLevelShortcuts, subChatStore])
  ...
  useEffect(() => {
    ...
    window.addEventListener("keydown", handleKeyDown, true)
    return () => window.removeEventListener("keydown", handleKeyDown, true)
  }, [acceptsTopLevelShortcuts, isDiffSidebarOpen])
            ...
            >
              ...
              {tabsToRender.length > 0 && agentChat ? (
                <div className="relative flex-1 min-h-0">
                  ...
                  ) : splitPaneIds.length >= 2 &&
                    !!activeSubChatId &&
                    splitPaneIds.includes(activeSubChatId) ? (
                    <SplitViewContainer
                            ...
                            content: (
                              ...
                              >
                                <ChatViewInner
                                  ...
                                  taskId={(agentChat as any)?.taskId ?? null}
                                  isArchived={isArchived}
                                  onRestoreWorkspace={handleRestoreWorkspace}
                                  existingPrUrl={agentChat?.prUrl}
                                  isActive={paneId === activeSubChatId}
                                  isSplitPane={true}
                                  acceptsGlobalShortcuts={acceptsTopLevelShortcuts}
