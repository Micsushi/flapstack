...
export const McpServersIndicator = memo(function McpServersIndicator({
  ...
  const getStatusIcon = (status: MCPServerStatus) => {
    switch (status) {
      case "connected":
        return <span className="w-2 h-2 rounded-full bg-green-500" aria-label="Connected" />
      case "failed":
        return <span className="w-2 h-2 rounded-full bg-red-500" aria-label="Connection failed" />
      case "needs-auth":
        return (
          <span className="w-2 h-2 rounded-full bg-yellow-500" aria-label="Needs authentication" />
        )
      case "pending":
        return (
          <Loader2 className="w-3 h-3 text-muted-foreground animate-spin" aria-label="Connecting" />
        )
      default:
        return (
          <span
