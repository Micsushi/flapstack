...
export const FilesTab = memo(
  forwardRef<FilesTabHandle, FilesTabProps>(function FilesTab(
    { worktreePath, onSelectFile, onExpandedStateChange, currentViewerFilePath, className },
    ref,
  ) {
    ...
    const visibleNodes = useMemo(() => flattenVisible(tree, expandedPaths), [tree, expandedPaths])
            ...
            >
              {tree.map((node) => (
                <TreeNode
                  ...
                  activePath={activePath}
                  isExpanded={
                    node.type === "folder" && !!node.children && expandedPaths.has(node.path)
                  }
                  editorLabel={editorLabel}
                  isPathExpanded={isPathExpanded}
