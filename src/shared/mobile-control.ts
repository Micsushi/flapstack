...
export const mobileActionCatalog = {
  ...
  "run.steer": {
    ...
    enabledByDefault: false,
    service: {
      module: "src/main/lib/agent-orchestration/operations-runtime.ts",
      operation: "CoordinationEngine.execute:send",
    },
  },
  "run.pause": {
    id: "run.pause",
    ...
    enabledByDefault: false,
    service: {
      module: "src/main/lib/main-run-launcher.ts",
      operation: "MainRuntimeLaunchService.pause",
    },
  },
  "run.resume": {
    id: "run.resume",
    ...
    service: {
      module: "src/main/lib/main-run-launcher.ts",
      operation: "MainRuntimeLaunchService.resume",
    },
  },
  "run.cancel": {
    id: "run.cancel",
    ...
    service: {
      module: "src/main/lib/main-run-launcher.ts",
      operation: "MainRuntimeLaunchService.cancel",
    },
  },
  "orchestration.pause": {
  ...
  "automation.pause": {
    ...
    enabledByDefault: false,
    service: {
      module: "src/main/lib/automation/execution.ts",
      operation: "AutomationExecutionService.pause",
    },
  },
  "automation.resume": {
    ...
    service: {
      module: "src/main/lib/automation/execution.ts",
      operation: "AutomationExecutionService.resume",
    },
  },
  "automation.cancel": {
    ...
    enabledByDefault: false,
    service: {
      module: "src/main/lib/automation/runtime.ts",
      operation: "AutomationExecutionService.kill",
    },
  },
  "approval.approve": {
    id: "approval.approve",
        ...
        .array(
          z
            .object({
              ...
              options: z.array(boundedText(200)).max(12),
...
const steerRunActionSchema = z
  .object({ type: z.literal("run.steer"), message: boundedText(10_000) })
  .strict()

export const mobileCommandActionSchema = z.discriminatedUnion("type", [
  clarificationAnswerActionSchema,
  steerRunActionSchema,
  targetOnlyAction("run.pause"),
  targetOnlyAction("run.resume"),
  targetOnlyAction("run.cancel"),
  targetOnlyAction("orchestration.pause"),
  targetOnlyAction("orchestration.resume"),
  targetOnlyAction("orchestration.cancel"),
  targetOnlyAction("automation.pause"),
  targetOnlyAction("automation.resume"),
  targetOnlyAction("automation.cancel"),
...
export const mobileCommandEnvelopeSchema = z
  .object({
    ...
    expiresAt: timestampSchema,
    scopeVersion: versionSchema,
    deviceConfirmedAt: timestampSchema,
    stepUpToken: z.string().trim().min(8).max(512).optional(),
  ...
  .superRefine((value, context) => {
    ...
    if (
      value.deviceConfirmedAt > value.issuedAt ||
      value.issuedAt - value.deviceConfirmedAt > mobileControlLimits.commandTtlMs
    )
      issue(context, ["deviceConfirmedAt"], "Device confirmation is stale or in the future.")
...
export type MobileResourceRef = z.infer<typeof mobileResourceRefSchema>
export type MobileTrustedTarget = MobileResourceRef & { version: number }
...
export function authorizeMobileCommand(
  ...
  input: {
    ...
    trustedTargetIsGranted?: boolean
  ...
  if (
    command.deviceConfirmedAt > command.issuedAt ||
    command.issuedAt - command.deviceConfirmedAt > mobileControlLimits.commandTtlMs
  )
    return denied("device-confirmation-required", "Fresh device confirmation is required.")
  if (
...
function actionTargetKind(action: MobileActionId): z.infer<typeof mobileResourceKindSchema> {
  ...
  if (action.startsWith("run.")) return "run"
