const COMPLETE_ENVELOPES = [
  ...
  /(^|\n)\[FLAPSTACK PRODUCT MCP\][\s\S]*?\[\/FLAPSTACK PRODUCT MCP\]\s*/gim,
...
const INCOMPLETE_ENVELOPE =
  /(^|\n)(?:\[(?:FLAPSTACK(?: STARTUP CONTEXT| PRODUCT MCP| DEFAULTS| THREAD DEFAULTS)|USER REQUEST|FILE:[^\]]*)\]|--- (?:FLAPSTACK (?:INTERNAL|COMPACT) CONTEXT(?: \(DO NOT QUOTE\))?|FLAPSTACK RESPONSE CONTRACT|USER REQUEST|BEGIN LOADED FILE:[^\n]*) ---)[\s\S]*$/i
...
export function sanitizeHarnessEnvelopeEcho(text: string): string {
  ...
  sanitized = sanitized.replace(
    /^\s*(?:\[\/(?:FLAPSTACK STARTUP CONTEXT|FLAPSTACK PRODUCT MCP|FLAPSTACK DEFAULTS|FLAPSTACK THREAD DEFAULTS|USER REQUEST|FILE)\]|--- END (?:FLAPSTACK (?:INTERNAL|COMPACT) CONTEXT|FLAPSTACK DEFAULTS|FLAPSTACK RESPONSE CONTRACT|USER REQUEST|LOADED FILE) ---)\s*$/gim,
    "",
  )
