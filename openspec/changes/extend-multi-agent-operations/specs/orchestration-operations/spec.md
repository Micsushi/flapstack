...
### Requirement: First-class reasoning and activity stream

Flapstack SHALL persist and render provider-exposed reasoning summaries, visible
reasoning, tool activity, workflow phases, subagent progress, warnings, and usage
with stable ordering and provenance without exposing opaque private reasoning.

#### Scenario: Codex emits indexed summary parts

- **WHEN** Codex streams summary deltas, section breaks, and raw-content metadata
- **THEN** Flapstack preserves thread, turn, item, summary/content indices, sequence,
  and visibility while rendering only provider-displayable content

#### Scenario: Provider exposes no reasoning text

- **WHEN** a run reports reasoning tokens or activity but no displayable reasoning
- **THEN** the UI shows tokens/activity or `Unavailable` and never invents thoughts

### Requirement: Independent reasoning and subagent controls

Flapstack SHALL configure reasoning effort, reasoning display, subagent text
forwarding, and hook diagnostics independently.

#### Scenario: Hide reasoning but show child progress

- **WHEN** reasoning display is disabled and subagent progress is enabled
- **THEN** child status and provider-exposed progress remain visible without
  rendering reasoning text or changing the model's configured effort
