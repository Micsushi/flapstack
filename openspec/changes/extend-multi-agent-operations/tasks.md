...
- Code-ready evidence: The read-only fleet base now applies scope, lifecycle, archive, provider, count, facet, sort, and keyset work in SQL before reading only the current page's agents. The 1,005-row fixture reads 25 agent rows for a 25-row page, preserves stable cursors, and retains explicit legacy-engine/provider/cost/freshness truth, scoped Tier-0 MCP reads, polling/invalidation, and accessible roving focus. Completion remains open for S4-F4-T6 operation-workspace navigation and live/package/provider evidence.
