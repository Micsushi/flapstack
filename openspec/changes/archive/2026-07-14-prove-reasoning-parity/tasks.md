...
- Exact Dev runs completed and reloaded for Claude, Codex, Cursor, OpenRouter,
  and NanoGPT. The mounted disclosures were inspected for Codex, Cursor, and
  OpenRouter; NanoGPT persisted only provider-visible output; Claude truthfully
  rendered no fabricated disclosure when none was available.
- Timer identity, remount, search, private-input exclusion, keyboard disclosure,
  and accessible expanded state agree between renderer control, persisted
  messages, and the sanitized captures. Bare Codex model plus effort now
  normalizes to the ACP-required model path and has focused regression coverage.
