...
- [x] Completion: acceptance and verification passed
- Status: selected ACP/App Server path implemented; focused tests, live smoke,
  and the same-model benchmark pass.
- Parent: Project Flapstack / Feature AQ-F4
...
- [x] Completion: acceptance and verification passed
- Status: permanent five-provider fixture, full check, and credentialed live
  Codex/Claude/Cursor/OpenRouter/NanoGPT smokes pass. Same-model Codex and Claude
  baselines pass.
...
2026-07-14 same-model benchmark: one disposable two-worktree repository used
`gpt-5.3-codex-spark` low and `claude-sonnet-5` low through both native CLIs and
Flapstack. The five-point hard-fact rubric scored completed, total, remaining,
integration branch, and worktree count. Native Codex and Flapstack Codex each
scored 5/5; native Claude and Flapstack Claude each scored 2/5 because both
independently rejected the deliberately answer-key-shaped numeric fields while
correctly reporting `integration` and two worktrees. Flapstack run IDs were
`64be0034-b694-49f4-9692-778c42b5a4d4` and
`6ac73841-40a0-4e8d-b6d3-0b7a2aa57010`. Both terminal runs were successful,
read-only, session-isolated, and persisted provider text, tool activity,
permission application, telemetry, and session metadata without envelope echo.
