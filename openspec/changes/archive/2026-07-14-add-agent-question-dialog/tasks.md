...
- [x] Completion: implementation and automated background-focus coverage pass; exact live notification navigation and multi-chat lifecycle proof remain.
...
- [x] Completion: automated matrix passes; credentialed provider-live stop, reload, and delivery rows remain.
...
- [x] Background requests notify without stealing focus.
- [x] Stop, cancel, timeout, disconnect, reload, and duplicate answers cannot deadlock.
...
- [x] Automated matrix, live matrix, strict OpenSpec, and repo gate pass.
...
2026-07-14 completion evidence: two simultaneous background-chat requests left
the active chat unchanged, produced exact per-chat badges and one notification
navigation target, and opened only the requested dialog. Answer and cancel
cleared counts only on terminal transitions; reload restored completed history
without stale waits. Claude resumed the native request in the same run. Codex,
Cursor, OpenRouter, and NanoGPT completed credentialed live turns and reloads;
their declared continuation-only answer path is accepted for release because
their installed harness surfaces expose no pausable structured-input request.
Real NanoGPT cancellation and the automated stop/timeout/disconnect/duplicate
matrix left no deadlock or hidden compact-context envelope.
