...
- Exact macOS Preview LaunchAgent closed-app start, heartbeat/poll, stop,
  new-PID restart, uninstall, and zero-job/plist/process cleanup passed. A
  credentialed OpenRouter sample and persisted Discord threshold alert were
  produced while the packaged app was closed; the webhook credential remained
  Keychain-backed and returned HTTP 204.
- The exact Dev Usage UI matched SQLite for provider filters, history ranges,
  paging, charts, alerts, and limitations. Codex, Anthropic, Cursor, and
  OpenRouter refresh passed; NanoGPT truthfully reported run-usage-only.
  Windows/Linux and organization Admin usage APIs are deferred release work.
